...
- テキストエディタ
	- Emacs
	- TeraPadはMacには無いので自分の慣れたやつ使ってください
- バイナリエディタ
	- 0xED
...
でまあシェルスクリプトの実行ですがターミナルを開いてください。ターミナルを知らない人は「アプリケーション＞ユーティリティ＞ターミナル」で見つかると思います。そして次の通りに実行してください。
...
みたいな桁になるようにすると書籍通りにできます。こうして最終的にできたのが`01_day/helloos/helloos.img`になります。

でまあもし自分の書いたやつを使いたいのであれば、`01_day/helloos/helloos.img`を上書きしてください。(不安なら上書き前の`helloos.img`をデスクトップとかに保管するとかして)
...
最初に18万行を超えるやつ書けとか言われます。著者には悪いですがスルーしましょう。めんどくさいだけですねｗ

次にまともな長さのソースプログラムってのがありますが、これは練習がてら書いてもいいかもしれません。`helloos.nas`というファイル名で書いてみてください。

最終的にできたのが`01_day/helloos1/helloos.nas`になります。自分で書いたやつを試したいのであれば上書き保存してください。(例によって不安なら上書きする前にサンプルのやつは保管しておきましょう)
...
ここも自分で書いたら`01_day/helloos2/helloos.nas`に上書き保存して実行してください。これまたmakeします。
...
`make`というのはこの「Makefile」を実行するためのものです。ちょっとのぞいてみましょう。`01_day/helloos2`のMakefileだと、
...
みたいになってます。でまあ、これは「`make run`と入力されたらrun : 以下の指定されたことをやりなさいよ」という指示になってます。指定されたことというのは、`../../z_tools/nask helloos.nas helloos.img`みたいなやつです。

これから先(27日目のharib24gまで)は、この`make run`でサンプルOSを実行できるようになってるので、よくわからなくても`make run`していけばサンプルは実行できます。そのように全てMac用に書き直しているからです。

なので困ることはないんですが、これから先やっていく上で不便なことやわからない部分が出てきます。本書でもMakefileの整理とかいって書き直しているページとか出てきますので、そのときにどうなってんのかちょっとばかりのぞいてみると良いと思います。
...
`02_day/helloos3/helloos.nas`のソースコードの解説です。一応`make run`できるようにしてますが、今までと同じ内容が起動されるだけなのでやらなくてもいいと思います。
...
本書ではここから本格的にMakefileが導入されます。どのようなコマンドが使えるのか本書を読みつつ、`02_day/helloos5/Makefile`をのぞいてみてください。`make run`以外にもいくつかありますよ。
...
こんな感じですね。`make src_only`が何なのかは本書を読んでください。最悪やってみるとわかりますよ。

### 簡易講座はここまでです

分からない人にとってはちょっと言葉足らずの内容だったかもしれません。しかし、このあたりのことは知識として持っていないとこれから先厳しいと思います。足りないなと思った人はちょっとがんばってください。

とりあえずこんな感じでサンプルを試しつつ学習してくことになります。

あと、自分でコードを書きながら学習していこうと考えている人は最悪Makefileのことが分からないといけません。Makefileが分からないという人は、サンプルのコードを読みながらサンプルを実行していく学習スタイルの方が良いと思います。
